...
|------------------------------------|---------------------------------|---------|-----------------------|
| Ability to control CPU load        | It is not a trivial task to control the CPU utilization with server request.                               | Not that easy to control the cpu cycles.  Using -cpu arguments we can specify the number of cpu stessors.    | Easy to control CPU consumption using simple curl command.                      | 
